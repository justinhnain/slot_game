...
p1 <- ggplot(longrun_spins, aes(payout)) +
  ...
  ylab("") +
  theme_minimal()
p2 <- openintro::normTail(M = c(qnorm(0.025), qnorm(0.975)), col = "skyblue")

p1 + p2 + 
  plot_layout(ncol = 2)
```

A 95% t-confidence interval for the payout per game is $0.957314 \pm 0.0086837$, which captures the theoretical expected payout of about $0.958$.
...
# 
# bootstrapped_means <- subset_spins |> 
#   rep_sample_n(size = 10000, replace = TRUE, reps = 10000) |> 
#   group_by(replicate) |> 
#   summarise(average_payout = mean(payout))
# 
# bootstrapped_means  |> 
#   ggplot(aes(average_payout)) +
#   geom_histogram() 
# 
# bootstrapped_means |>
#   pull(average_payout) |> 
#   quantile(c(0.025, 0.975))
# mean(bootstrapped_means$average_payout)
```
