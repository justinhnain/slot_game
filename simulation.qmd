...
outcome_df <- expand.grid(py$x1, py$x2, py$x3) |> 
  as_tibble()
...
Then, we simulate `outcome`s from $N_{spins}$ spins across $N_{sessions}$ sessions by sampling from $\Omega\text{'s}$ dataframe with replacement.
